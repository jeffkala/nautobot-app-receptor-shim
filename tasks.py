...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_RECEPTOR_SHIM_xxx
namespace = Collection("receptor_shim")
namespace.configure(
    {
        "receptor_shim": {
            "nautobot_ver": "2.1.0",
            "project_name": "receptor-shim",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.receptor_shim.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.receptor_shim.compose_http_timeout,
        "NAUTOBOT_VER": context.receptor_shim.nautobot_ver,
        "PYTHON_VER": context.receptor_shim.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.receptor_shim.project_name}",
        f'--project-directory "{context.receptor_shim.compose_dir}"',
    ]

    for compose_file in context.receptor_shim.compose_files:
        compose_file_path = os.path.join(context.receptor_shim.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.receptor_shim.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.receptor_shim.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations receptor_shim"
...
@task(
    help={
        "version": "Version of Receptor Shim to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml receptor_shim'
    run_command(context, command)
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'receptor_shim/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.receptor_shim.local):
